...
def pyrouge_score(preds, labels, num_processes=16):
    ...
    res_dict = {'rouge-1': results['rouge_1_f_score'] * 100, 'rouge-2': results['rouge_2_f_score'] * 100,
                'rouge-L': results['rouge_l_f_score'] * 100}
    print(res_dict)
    return res_dict


def get_rouge_score(sys_path, ref_path):
    predictions = []
    golds = []
    for inst in sys_outputs:
        ...
        gold = ref_dict[inst["article_id"]]
        assert isinstance(sys_out, str)
        assert isinstance(gold, str)
        predictions.append(sys_out)
        golds.append(gold)

    # files2rouge.run(sys_out_file, ref_out_file, saveto="rouge.txt")
...
if __name__ == '__main__':
    ...
    for cand_file in candidate_files:
        ...
        get_rouge_score(sys_path, ref_path)
        # get_bleu_score()
